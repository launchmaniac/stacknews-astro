...
function StatCard({ label, value, prefix, subValue, trend, color, children, tooltip }: {
  ...
  return (
    <div className="glass-panel p-4 rounded-xl relative overflow-hidden group font-mono flex flex-col justify-between h-full min-h-[140px]" title={tooltip}>
        ...
        <div className="glass-panel p-4 rounded-xl relative overflow-hidden group font-mono flex flex-col justify-between h-full min-h-[140px]">
          ...
          <div className="relative z-10 flex flex-col h-full justify-between">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h4 className="text-[11px] font-thin uppercase tracking-widest text-cyan-400/80" title="Sum of all outstanding Treasury securities and intragovernmental debt">
                Total Public Debt
              </h4>
            </div>

            <div className="text-2xl md:text-3xl font-medium mb-1" style={{ textShadow: '0 0 20px rgba(239, 68, 68, 0.25)' }}>
              <DebtCounter compact={false} showRate={true} className="text-white" />
            </div>
            ...
            <div className="flex items-center gap-2 text-sm mt-auto pt-1">
              ...
              <span className="text-gray-400 font-thin text-xs opacity-80">INCREASING</span>
            </div>
          </div>
          ...
          <div className="absolute -bottom-10 -right-10 w-32 h-32 rounded-full blur-[60px] opacity-20 pointer-events-none bg-red-500" />
        </div>
      </div>
      ...
      <div className="glass-panel p-4 rounded-xl relative overflow-hidden group font-mono flex flex-col justify-between h-full min-h-[140px]">
        ...
        <div className="relative z-10 flex flex-col h-full justify-between">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h4 className="text-[11px] font-thin uppercase tracking-widest text-cyan-400/80" title="Treasury's available cash balance for government operations">
              Operating Cash
            </h4>
          </div>
          ...
          <div
            className="text-2xl md:text-3xl font-medium mb-1 whitespace-nowrap overflow-hidden text-ellipsis"
            style={{ color: 'white', textShadow: '0 0 20px rgba(34, 197, 94, 0.25)' }}
          >
            <span className="text-[0.7em] opacity-60 mr-1 align-top font-normal">$</span>
            {data && data.cash > 0 ? formatCash(data.cash) : '--'}
          </div>

          <div className="flex flex-col gap-0.5 mt-auto pt-1">
            <span className="text-gray-400 font-thin text-xs opacity-80" title="Current month's operating cash available for expenditures">LIQUID ASSETS</span>
            {data?.cashHistoricalAvg && data.cashHistoricalAvg > 0 && (
              <span className="text-gray-500 font-thin text-[10px] opacity-70" title="Average Treasury operating cash over past 52 weeks">
                52-wk avg: ${formatCash(data.cashHistoricalAvg)}
              </span>
            )}
          </div>
        </div>

        <div className="absolute -bottom-10 -right-10 w-32 h-32 rounded-full blur-[60px] opacity-20 pointer-events-none bg-green-500" />
      </div>

      <StatCard
        label="Avg Interest Rate"
        tooltip="Weighted average interest rate paid on all outstanding Treasury debt"
