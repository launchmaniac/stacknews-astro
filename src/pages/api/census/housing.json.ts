...
// Census EITS Housing (starts, permits) with Cache API + KV fallback

import type { APIRoute } from 'astro';
import { getHousingStats } from '../../../lib/census';
import type { HousingStats } from '../../../lib/types';
...
export const GET: APIRoute = async ({ request, url, locals }) => {
  ...
  const KV_KEY = 'census:housing:v1';
  ...
  try {
    ...
    const data: HousingStats = await getHousingStats(24);
    const res = new Response(
      JSON.stringify({ ...data, _cache: { hit: false }, timestamp: new Date().toISOString() }),
      { status: 200, headers: { 'Content-Type': 'application/json', 'Cache-Control': 'public, s-maxage=300, stale-while-revalidate=600, stale-if-error=86400', 'X-Cache': 'MISS' } }
    );
    ...
    const kvData = await kvGetJSON<{ data: HousingStats }>(kv, KV_KEY);
    if (kvData?.data) {
      return new Response(
    ...
    return new Response(JSON.stringify({ error: 'Failed to fetch housing stats', message: error?.message }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
};
